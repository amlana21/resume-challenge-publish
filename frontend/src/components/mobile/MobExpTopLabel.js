...
const NeumorphShadow = (props) => {
    ...
    return (
      <>
        ...
        >
          <Box style={{position:'absolute',left:'2px'}}><IconButton aria-label="back">
          <ArrowBackIcon fontSize='large' onClick={()=>props.backBtn()} />
      </IconButton></Box>
          <Box classes={classes} style={{backgroundColor:'#686d76',backgroundImage: "url(" + topLabelBtnback + ")"}} borderRadius={16} width={240} height={70} justifyContent={'center'} alignItems={'center'}>
          <div style={{position:'relative',margin:'auto',left:'15%'}}><Typography variant="h3" gutterBottom className='mobaboutmetoplabel'>
    {props.bodytext}
  </Typography></div>
